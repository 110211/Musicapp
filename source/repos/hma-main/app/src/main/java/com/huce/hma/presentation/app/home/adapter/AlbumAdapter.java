...
public class AlbumAdapter extends ArrayAdapter<SongSearchResultDTO> {
    Context context;
    ArrayList<SongSearchResultDTO> arrayList;
    int layoutResource;
    //Hàm khởi tạo cho CustomArrayAdapter
    public AlbumAdapter(Context context, int resource, ArrayList<SongSearchResultDTO> objects) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        TextView songName = (TextView)convertView.findViewById(R.id.album_name);
        songName.setText(arrayList.get(position).getName());

        TextView txt2 = (TextView)convertView.findViewById(R.id.album_maker);
        txt2.setText(arrayList.get(position).getArtistName());

        ImageView img = (ImageView) convertView.findViewById(R.id.album_image);
